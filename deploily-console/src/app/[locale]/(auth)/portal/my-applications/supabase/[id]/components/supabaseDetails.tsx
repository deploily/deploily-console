      ...
      {!isLoading && supabaseAppById !== undefined && (
        <>
                ...
                >
                  <Typography.Title level={2} style={{ color: theme.token.orange400 }}>
                    {Intl.NumberFormat("fr-FR", { useGrouping: true }).format(
                      supabaseAppById.total_amount / supabaseAppById.duration_month,
                    )}{" "}
                    DZD /{" "}
                    {supabaseAppById.service_plan.subscription_category === "monthly"
                      ? t("month")
                      : t("year")}
                  </Typography.Title>
                </Col>
                <DocumentationComponents
                  supabaseAppById={supabaseAppById}
          ...
          <DocumentationDrawer
            ...
            onClose={onClose}
            currentSubscription={supabaseAppById}
            t={t}
          />
        </>
      )}
