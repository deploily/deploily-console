...
export default function PlanDetailsComponent({ supabaseAppById }: { supabaseAppById: any }) {
    const t = useI18n();

    return (
        <>
            {supabaseAppById.get_plan_details && (
                <Collapse
                    bordered={false}
                    defaultActiveKey={["0"]}
                    expandIcon={({ isActive }) => (
                    ...
                    items={[
                        {
                            ...
                            children: (
                                <>
                                        ...
                                        <Col xs={24} md={8}>
                                            <Row align="middle" gutter={[8, 0]}>
                                                ...
                                                <Col flex="auto">
                                                    ...
                                                    >
                                                        {supabaseAppById.get_plan_details.plan?.name ||
                                                            supabaseAppById.name}
                                                    </Typography.Text>
                                                </Col>
                                            </Row>
                                        </Col>
                                    ...
                                    {/* === PLAN OPTIONS === */}
                                    {Array.isArray(supabaseAppById.get_plan_details.options) &&
                                        supabaseAppById.get_plan_details.options.length > 0 && (
                                            <Col span={24} style={{ marginTop: 10 }}>
                                                ...
                                                <div style={{ marginTop: 10 }}>
                                                    {supabaseAppById.get_plan_details.options.map(
                                                        (option: any) => (
                                                            <div
                                                                key={option.id}
                                                                style={{
