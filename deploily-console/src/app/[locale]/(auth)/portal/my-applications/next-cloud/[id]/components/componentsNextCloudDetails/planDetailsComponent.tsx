...
export default function PlanDetailsComponent({ nextCloudAppById }: { nextCloudAppById: any }) {
    const t = useI18n();

    return (
        <>
            {nextCloudAppById.get_plan_details && (
                <Collapse
                    bordered={false}
                    defaultActiveKey={["0"]}
                    expandIcon={({ isActive }) => (
                    ...
                    items={[
                        {
                            ...
                            children: (
                                <>
                                        ...
                                        <Col xs={24} md={8}>
                                            <Row align="middle" gutter={[8, 0]}>
                                                ...
                                                <Col flex="auto">
                                                    ...
                                                    >
                                                        {nextCloudAppById.get_plan_details.plan?.name ||
                                                            nextCloudAppById.name}
                                                    </Typography.Text>
                                                </Col>
                                            </Row>
                                        </Col>
                                    ...
                                    {/* === PLAN OPTIONS === */}
                                    {Array.isArray(nextCloudAppById.get_plan_details.options) &&
                                        nextCloudAppById.get_plan_details.options.length > 0 && (
                                            <Col span={24} style={{ marginTop: 10 }}>
                                                ...
                                                <div style={{ marginTop: 10 }}>
                                                    {nextCloudAppById.get_plan_details.options.map(
                                                        (option: any) => (
                                                            <div
                                                                key={option.id}
                                                                style={{
