...
export default function DurationComponent({ odooAppById }: { odooAppById: any }) {
    const t = useI18n();
    const [remainingDuration, setRemainingDuration] = useState<number>()

    useEffect(() => {
        if (odooAppById !== undefined) {
            setRemainingDuration(getRemainingDuration(odooAppById.start_date, odooAppById.duration_month));
        }
    }, [odooAppById]);
