...
export default function ApplicationPlansContainer({ screens }: { screens: ReturnType<typeof Grid.useBreakpoint> }) {
  ...
  const partialVisible = (servicePlanResponse?.result?.length ?? 0) > 1;
            ...
            {servicePlanResponse.result.map((plan) => (
              <div
                ...
                style={{
                  ...
                  display: "flex",     // ← needed for child to fill
                }}
              >
                <PlanCard
                  id={plan.id}
