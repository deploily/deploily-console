...
export default function ResourceDetailsContentPage({resource_id}: {resource_id: string}) {
  ...
  const showDrawer = (plan: any | null) => {
    if (plan !== null) {
      ...
      dispatch({
        type: "ApiServiceSubscriptionStates/updateApiServiceSubscriptionStates",
            ...
            >
              ...
              <Col style={{flexDirection: "column", justifyContent: "start"}}>
                ...
                <Typography.Title level={4} style={{color: theme.token.orange400, margin: 0}}>
                  {currentResource.unit_price !== undefined
                    ? Intl.NumberFormat("fr-FR", {useGrouping: true}).format(
                        currentResource.unit_price,
                      ) +
                      " DZD " +
                      " / " +
                      (currentResource.price_category == "monthly" ? t("month") : t("year"))
