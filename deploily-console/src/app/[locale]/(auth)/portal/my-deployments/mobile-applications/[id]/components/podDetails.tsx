...
const PodsDetails = ({ mobileApplicationById, planNames, theme, handleCopy, onSave }: {
    ...
    const plan = mobileApplicationById.get_plan_details.plan.name;
    ...
    const podNamesAndUrls = Array.from({ length: podCount }, (_, index) => ({
        name: mobileApplicationById[`pod_name_${index + 1}`] ?? 'name',
        url: mobileApplicationById[`pod_url_${index + 1}`] ?? 'url'
    }));
