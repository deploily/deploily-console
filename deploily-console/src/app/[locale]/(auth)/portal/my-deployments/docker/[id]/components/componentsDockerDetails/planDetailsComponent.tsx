...
export default function PlanDetailsComponent({ dockerById }: { dockerById: any }) {
    const t = useI18n();

    return (
        <>
            {dockerById.get_plan_details && (
                <Collapse
                    bordered={false}
                    defaultActiveKey={["0"]}
                    expandIcon={({ isActive }) => (
                    ...
                    items={[
                        {
                            ...
                            children: (
                                <>
                                        ...
                                        <Col xs={24} md={8}>
                                            <Row align="middle" gutter={[8, 0]}>
                                                ...
                                                <Col flex="auto">
                                                    ...
                                                    >
                                                        {dockerById.get_plan_details.plan?.name ||
                                                            dockerById.name}
                                                    </Typography.Text>
                                                </Col>
                                            </Row>
                                    ...
                                    {/* === PLAN OPTIONS === */}
                                    {Array.isArray(dockerById.get_plan_details.options) &&
                                        dockerById.get_plan_details.options.length > 0 && (
                                            <Col span={24} style={{ marginTop: 10 }}>
                                                ...
                                                <div style={{ marginTop: 10 }}>
                                                    {dockerById.get_plan_details.options.map(
                                                        (option: any) => (
                                                            <div
                                                                key={option.id}
                                                                style={{
