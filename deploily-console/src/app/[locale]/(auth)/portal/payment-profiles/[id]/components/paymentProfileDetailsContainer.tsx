  ...
  return (
    <div style={{paddingInline: 10}}>
      ...
      {currentProfileLoading && !currentProfile && (
        <Row gutter={[10, 10]}>
          <Col span={2}>
            <Skeleton active paragraph={{rows: 0.5}} />
          </Col>
          <Col span={22}>
      ...
      {!currentProfileLoading && currentProfile && (
        <>
          <Row style={{padding: 20, alignItems: "center"}} gutter={8}>
            ...
            <Col>
              <span style={{color: "white", fontWeight: 700}}>{t("balance")}:</span>
            </Col>
            <Col>
              <CustomTypography style={{color: "#D85912"}}>
                {Intl.NumberFormat("fr-FR", {useGrouping: true}).format(currentProfile?.balance)}{" "}
                DZD
              </CustomTypography>
            </Col>
          </Row>
          ...
          >
            ...
            <Form.Item label={t("wilaya")} name="wilaya">
              <CustomPayementInput
                defaultValue={currentProfile.wilaya}
                style={{color: theme.token.colorWhite}}
              />
            </Form.Item>
            ...
            {currentProfile.is_company ? (
                ...
                <Row gutter={[16, 16]}>
                  <Col md={12} xs={24}>
                    <Form.Item label={t("nif")} name="nif">
                      <CustomPayementInput
                        defaultValue={currentProfile.nif}
                        style={{color: theme.token.colorWhite}}
                      />
                    </Form.Item>
                  </Col>
                  <Col md={12} xs={24}>
                    <Form.Item label={t("nis")} name="nis">
                      <CustomPayementInput
                        defaultValue={currentProfile.nis}
                        style={{color: theme.token.colorWhite}}
                      />
                    </Form.Item>
                  </Col>
              ...
              <Form.Item label={t("nin")} name="nin">
                <CustomPayementInput
                  defaultValue={currentProfile.nin}
                  style={{color: theme.token.colorWhite}}
                />
              </Form.Item>
            )}
            ...
            <Form.Item wrapperCol={{span: 24}} style={{textAlign: "end", marginTop: 20}}>
              <CustomOrangeButton htmlType="submit">{t("save")}</CustomOrangeButton>
            </Form.Item>
          </Form>
        </>
      )}
      ...
      {!currentProfileLoading && currentProfileError && (
        <Result status="500" title={traslate("error")} subTitle={traslate("subTitleError")} />
      )}
