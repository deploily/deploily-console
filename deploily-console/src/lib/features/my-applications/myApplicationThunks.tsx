...
export const fetchMyApplications = createAsyncThunk(
    ...
    async (_, thunkConfig) => {
      try {
        ...
        const response = await axiosInstance.get(`${deploilyApiUrls.APPS_SERVICE_SUBSCRIPTION_URL}`, {
          headers: {
  ...
  export const fetchMyApplicationById = createAsyncThunk(
    ...
    async (my_app_id: string, thunkConfig) => {
        try {
            ...
            const response = await axiosInstance.get(`${deploilyApiUrls.APPS_SERVICE_SUBSCRIPTION_URL}${my_app_id}`, {
                headers: {
                    Accept: "application/json",
                    Authorization: `Bearer ${token}`,
                },
