    ...
    switch (action.type) {
      ...
      case "ressourcePlans/getResourceServicesPlans/fulfilled":
        if (action.payload !== undefined && action.payload.result.length > 0) {
          store.dispatch(
            updateNewAppSubscriptionState({managed_ressource_details: action.payload.result[0]}),
          );
          store.dispatch(
            updateUpgradeRenewMyAppState({managed_ressource_details: action.payload.result[0]}),
          );
          store.dispatch(
            updateNewDeploymentSubscriptionState({
              managed_ressource_details: action.payload.result[0],
            }),
          );
          //TODO add of upgrade & renew
        }
