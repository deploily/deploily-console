  ...
  async (resource_id: string, thunkConfig) => {
    try {
      ...
      const response = await axiosInstance.get(
        `${deploilyApiUrls.SERVICE_RESSOURCE}${resource_id}`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${token}`,
...
export const fetchResourceCategories = createAsyncThunk(
  ...
  async (_, thunkConfig) => {
    try {
      const response = await axiosInstance.get(`${deploilyApiUrls.RESOURCE_CATEGORY_URL}`, {});
      if (response.status == 200) {
        return response.data;
      } else {
        return thunkConfig.rejectWithValue("error");
...
export const getManagedResources = createAsyncThunk(
  ...
  async (_, thunkConfig) => {
    try {
      ...
      const managedResourceFilterParams=state.cloudResource.managedResourceFilterParams;

      const filters = `(page_size:${managedResourceFilterParams.page_size},page:${managedResourceFilterParams.page})`;
      const query = `?q=${encodeURIComponent(filters)}`;

      const response = await axiosInstance.get(`${deploilyApiUrls.MANAGED_RESSOURCE_URL}${query}`, {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
        },
